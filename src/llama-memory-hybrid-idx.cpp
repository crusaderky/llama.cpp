...
llama_memory_hybrid_idx::llama_memory_hybrid_idx(
    ...
    mem_idx(filter_idx == nullptr ? nullptr : [&] {
        ...
        return new llama_kv_cache(
            ...
            /* tail_tokens_requested  */ UINT32_MAX,
...
llama_memory_context_ptr llama_memory_hybrid_idx::init_batch(llama_batch_allocr & balloc, uint32_t n_ubatch, bool embd_all) {
    ...
    do {
        ...
        // prepare the attention cache; the indexer adopts this slot layout
        auto * mem_attn_kv = get_mem_attn_kv();
        if (!mem_attn_kv) {
            LLAMA_LOG_ERROR("%s: the hybrid-idx memory requires a plain KV attention cache\n", __func__);
            return std::make_unique<llama_memory_hybrid_idx_context>(LLAMA_MEMORY_STATUS_FAILED_PREPARE);
        }

        auto heads_attn = mem_attn_kv->prepare(ubatches);
        if (heads_attn.empty()) {
...
void llama_memory_hybrid_idx::state_read(llama_io_read_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) {
    ...
    try {
        if ((flags & LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY) == 0) {
            auto * mem_attn_kv = get_mem_attn_kv();
            if (!mem_attn_kv) {
                throw std::runtime_error("the hybrid-idx memory requires a plain KV attention cache");
            }
            mem_attn_kv->state_read_sinfo(io, seq_id, flags, mem_idx ? &sinfos_attn : nullptr, nullptr);
        }
...
llama_memory_hybrid_idx_context::llama_memory_hybrid_idx_context(
    ...
    llama_memory_hybrid_context(mem,
        std::make_unique<llama_kv_cache_context>(mem->get_mem_attn_kv(), std::move(sinfos_attn), ubatches),
        ubatches),
    mem(mem),
